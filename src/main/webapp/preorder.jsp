...
<%@ page import="com.foodparadise.model.PreOrderCartItem" %>
...
<head>
    <meta charset="UTF-8">
    <title>Your Pre-Order Cart — Food Paradise</title>
    <link rel="stylesheet" href="assets/preorderCart.css?v=1.0">
</head>
<body>
<header class="header">
    <nav class="navbar">
...
<main class="container no-print">
    <h2>Items in Your Pre-Order Cart</h2>

    <% if(isDecemberDiscount) { %>
    <div class="december-discount-banner">
        <h3>🎄 December Special Offer! 🎄</h3>
        <p>Enjoy 10% OFF on all pre-orders during December!</p>
    </div>
    <% } %>
    ...
    <div class="december-discount-banner" style="background: linear-gradient(135deg, #17a2b8, #28a745);">
        ...
        <p style="color: white;">Enjoy 20% OFF on all pre-orders during the rainy season!</p>
    </div>
    <% } %>
    ...
    <% } else { %>
    <form id="cartForm" action="AddPreOrderServlet" method="post">
        <!-- Expected Delivery Date for the whole order -->
<div class="expected-date-field">
    ...
    <input type="date" id="expectedDate" name="expectedDate"
           ...
           min="<%= LocalDate.now().plusDays(3) %>" required>
</div>
        ...
        <div class="cart-grid">
            ...
            for(int i=0; i<cart.size(); i++){
                PreOrderCartItem item = cart.get(i);
            ...
            <div class="cart-card" data-index="<%=i%>">
                ...
                <div class="cart-card-footer">
                    <button type="button" class="remove-btn"
                            onclick="window.location='RemovePreOrderCartItemServlet?index=<%=i%>'">
                        Remove
                    </button>
        ...
        <div class="button-group">
            ...
            <button type="button" class="checkout-btn" id="checkoutBtn" disabled onclick="confirmKBZPayment()">Pre-Order with KBZpay</button>
        </div>
    </form>
    <% } %>
</main>
...
<div class="printable-receipt" id="printableReceipt">
    <div class="receipt-header">
        ...
        <p>Expected Delivery Date: <span id="receiptExpectedDate"></span></p>
    </div>
...
function updateReceiptData() {
    ...
    const expectedDateValue = document.getElementById('expectedDate')?.value || 'Not set';
    ...
    document.getElementById('receiptExpectedDate').textContent = expectedDateValue;
